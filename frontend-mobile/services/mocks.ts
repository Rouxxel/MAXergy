...
export const mockAdvisorReply = (msg: string): AdvisorChatResponse => {
  ...
  let message = advisorReplies[0];
  if (lower.includes("financing") || lower.includes("term")) {
    message = advisorReplies[2];
  } else if (lower.includes("why") || lower.includes("config")) {
    message = advisorReplies[1];
  }

  return {
    advisor_message: message,
    context_used: ["assessment_data", "solar_pv_sizing"],
    suggestions: [
      "Can I change the battery size?",
      "What are the payback terms?",
      "How is solar tilt calculated?"
    ]
  };
};